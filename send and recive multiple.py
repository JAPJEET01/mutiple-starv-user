...
RECEIVER_IPS = ['192.168.41.208', '192.168.41.209', '192.168.41.210']  # List of receiver IP addresses
...
receiver_streams = [audio.open(format=FORMAT, channels=CHANNELS, rate=RATE, output=True, frames_per_buffer=CHUNK)
                    for _ in range(len(RECEIVER_IPS))]

# Set up sender and receiver sockets
sender_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
receiver_sockets = [socket.socket(socket.AF_INET, socket.SOCK_DGRAM) for _ in range(len(RECEIVER_IPS))]
